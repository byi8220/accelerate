...
def create_data_loader_shard(
    ...
    **kwargs,
    ...
    """Create a `DataLoader` or a `DataLoaderShard` depending on the `use_stateful_dataloader` flag."""
    if use_stateful_dataloader and is_torchdata_stateful_dataloader_available():
        from .stateful_data_loader import StatefulDataLoaderShard
        return StatefulDataLoaderShard(
            dataset, 
            device=device, 
            rng_types=rng_types, 
...
class DataLoaderDispatcher(DataLoader, DataLoaderStateMixin):
    """
    Subclass of a PyTorch `DataLoader` that will iterate and preprocess on process 0 only, then dispatch on each process
...
def create_data_loader_dispatcher(
    dataset,
    split_batches: bool = False,
    skip_batches=0,
    use_stateful_dataloader = False,
    ...
    slice_fn=None,
    **kwargs):
    """Create a `DataLoader` or a `DataLoaderDispatcher` depending on the `use_stateful_dataloader` flag."""
    if use_stateful_dataloader and is_torchdata_stateful_dataloader_available():
        from .stateful_data_loader import StatefulDataLoaderDispatcher
...
def prepare_data_loader(
    ...
    if dispatch_batches:
        ...
        dataloader = create_data_loader_dispatcher(
            new_dataset,
            split_batches=split_batches,
        ...
        dataloader = create_data_loader_shard(
            new_dataset,
...
def create_skip_data_loader(dataset, skip_batches= 0, use_stateful_dataloader = False, **kwargs):
    """Create a `DataLoader` or a `SkipDataLoader` depending on the `use_stateful_dataloader` flag."""
    if use_stateful_dataloader and is_torchdata_stateful_dataloader_available():
        from .stateful_data_loader import StatefulSkipDataLoader
        return StatefulSkipDataLoader(dataset, skip_batches=skip_batches, **kwargs)
    else:
        return SkipDataLoader(dataset, skip_batches=skip_batches, **kwargs)

def is_data_loader_dispatcher(obj: Any) -> bool:
    if is_torchdata_stateful_dataloader_available():
        from .stateful_data_loader import StatefulDataLoaderDispatcher
        return isinstance(obj, StatefulDataLoaderDispatcher) or isinstance(obj, DataLoaderDispatcher)
    return isinstance(obj, DataLoaderDispatcher)

def is_data_loader_shard(obj: Any) -> bool:
    if is_torchdata_stateful_dataloader_available():
        from .stateful_data_loader import StatefulDataLoaderShard
        return isinstance(obj, StatefulDataLoaderShard) or isinstance(obj, DataLoaderShard)
    return isinstance(obj, DataLoaderShard)

def is_skip_data_loader(obj: Any) -> bool:
    if is_torchdata_stateful_dataloader_available():
        from .stateful_data_loader import StatefulSkipDataLoader
        return isinstance(obj, StatefulSkipDataLoader) or isinstance(obj, SkipDataLoader)
    return isinstance(obj, SkipDataLoader)
...
def skip_first_batches(dataloader, num_batches=0):
    ...
    if is_data_loader_dispatcher(dataloader):
        if new_batch_sampler is None:
    ...
    elif is_data_loader_shard(dataloader):
        if new_batch_sampler is None:
            # Need to manually skip batches in the dataloader
            kwargs["skip_batches"] = num_batches
        elif sampler_is_batch_sampler:
